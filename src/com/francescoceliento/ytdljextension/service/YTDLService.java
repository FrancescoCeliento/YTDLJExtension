...
public class YTDLService {
	...
	public List<YTDLItem> getIdList() {
		...
		if (!input.isFiltered()) {
			System.out.println("Non � stata impostato un filtro sulle date, saranno scaricati tutti gli item rilevati");
			return itemList;
		}
	...
	private List<YTDLItem> getInfoPlaylist(List<String> idList) {
		...
		if (resultList.size() != idList.size()) {
			System.out.println("ATTENZIONE: Il numero di ID scaricati non equivale al numero di ID per la quale abbiamo ricevuto le informazioni. � possibile che tu abbia perso la connessione ad internet durante l'interrogazione. Premerere INVIO per continuare oppure CTRL+C per fermare la procedura e riavviarla.");
			try {
				System.in.read();
			} catch (IOException e) {
		...
		//Chiudo il database perch� non mi serve pi�
		database.close();
	...
	public void runDownload(List<YTDLItem> items, boolean ifSplitting, YTDLInput input) {
		...
		for (YTDLItem ytdlItem : items) {
			...
			if (checkExistingFile(input.getDownloadDirectory(),ytdlItem.getFileName())) {
				System.out.println("Il file " + ytdlItem.getFileName() + " esiste gi�");
			} else if (ytdlItem.getId().equals("Oaq9uRxsJhQ")) {
				//TODO: Aggiungere tabella esclusioni
				System.out.println("Il file " + ytdlItem.getFileName() + " � escluso dal download");
			} else if (!repo.download(ytdlItem,ifSplitting)) {
				System.out.println("Qualcosa � andato storto, inserisco il comando di download nel file di log");
